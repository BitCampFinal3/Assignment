...
# 팽이가 전날까지 가야 할 높이: v-a
# 이 높이를 (a-b)로 나누는데, 나누어 떨어지지 않을 경우에는 하루 더 가야함.

day = (v-a) // (a-b)

if (v-a)%(a-b) == 0:
    # 여기에서 default값으로 day에 +1을 하는 것은 전날까지 올라야 할 높이에 대해서만 계산했기 때문.
    print(day+1)
else:
    print(day+2)
...
# 위와 같이 1,2트에서는 반복문을 사용했음.
# 이 방법은 논리적으로는 맞지만 큰 수에 대해서는 시간 초과가 뜨므로 
# 가급적 반복문은 피해야 함!!
